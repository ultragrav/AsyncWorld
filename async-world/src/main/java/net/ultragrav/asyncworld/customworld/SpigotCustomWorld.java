...
public class SpigotCustomWorld extends CustomWorld {
    ...
    private final ChunkSnapCache chunkSnapCache = new ChunkSnapCache();
    ...
    public synchronized void create(SavedCustomWorld world, boolean preloadChunks, World.Environment environment) {
        ...
        if (!preloadChunks) {
            this.chunkSnapCache.clear();
            this.chunkSnapCache.load(world.getChunks());
        }

        //Create world server
        if (worldHandler == null) createWorldHandler();
        if (!worldHandler.isWorldCreated()) worldHandler.createWorld(this, name, environment);

        CompletableFuture<Void> worldListFuture = new CompletableFuture<>();
        ...
        if (preloadChunks) {
            ...
            world.getChunks().forEach(c -> this.asyncWorld.getChunk(c.getX(), c.getZ()));
            ...
            Map<Long, CustomWorldChunkSnap> chunkSnapMap = new ConcurrentHashMap<>(world.getChunks().size());
            world.getChunks().forEach(c -> chunkSnapMap.put(ChunkSnapCache.getKey(c.getX(), c.getZ()), c));

            chunks.forEach((c) -> pool.submit(() -> {
                CustomWorldChunkSnap snap = chunkSnapMap.get(ChunkSnapCache.getKey(c.getLoc().getX(), c.getLoc().getZ()));
                if (snap != null) c.fromSnap(snap);
                worldHandler.finishChunk(c);
            })); //Submit tasks
    ...
    public void unloadAndSaveChunk(int cx, int cz) {
        ...
        if (chunk != null) {
            ...
            // Take a snapshot of the chunk
            CustomWorldChunkSnap snap = CustomWorldChunkSnap.fromAsyncChunk(chunk, runnable -> {
                CompletableFuture<Void> future = new CompletableFuture<>();
                SyncScheduler.sync(() -> {
                    runnable.run();
                    future.complete(null);
    ...
    private SavedCustomWorld save(SpigotCustomWorldAsyncWorld asyncWorld, boolean asyncIsSafe) {
        ...
        Map<Long, CustomWorldChunkSnap> copiedSnaps = chunkSnapCache.save();

        // Define a sync executor
        ...
        // For each chunk in the async world, take a snapshot of it and add it to the save
        ...
        // Shutdown the pool we used to multi-thread it and wait for it's completion
        pool.shutdown();
        while (true) {
            try {
        ...
        // Add all the chunks that were unloaded (if preloaded = false)
        if (!preloaded.get()) {

            // Prioritize in-use chunks over saved unloaded ones by removing
            // the ones we've already saved from the in-use ones from the cached ones
            for (CustomWorldChunkSnap chunk : save.getChunks()) {
                copiedSnaps.remove(ChunkSnapCache.getKey(chunk.getX(), chunk.getZ()));
            }
